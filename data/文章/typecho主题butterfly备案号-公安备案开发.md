...
if (empty($vai['1']['0'])) {
    ...
    if($dbk == NULL){
        ...
        $qqurl = file_genuoyis_typecho_contents($geturl);
...
//  回复可见       
Typecho_Plugin::factory('Widget_Abstracnuoyis_typecho_contents')->excerptEx = array('myyodux','one');
Typecho_Plugin::factory('Widget_Abstracnuoyis_typecho_contents')->contentEx = array('myyodux','one');
class myyodux {
    ...
    if (isset($_REQUEST['text']) != null) {
		...
		$secretKey = Helper::options()->secretKey;
			function getCaptcha($recaptcha_response, $secretKey) {
				$response = file_genuoyis_typecho_contents("https://recaptcha.net/recaptcha/api/siteverify?secret=".$secretKey."&response=".$recaptcha_response);
...
if (isset($_REQUEST['text']) != null) {
    if($_POST['h-captcha-response'] == null) {
     ...
     if(isset($_POST['h-captcha-response']) && !empty($_POST['h-captcha-response'])){
         ...
         $verifyResponse = file_genuoyis_typecho_contents('https://hcaptcha.com/siteverify?secret='.$secret.'&response='.$_POST['h-captcha-response'].'&remoteip='.$_SERVER['REMOTE_ADDR']);
...
// 微博热搜
function weibohot(){
$api = file_genuoyis_typecho_contents('https://weibo.com/ajax/side/hotSearch');
